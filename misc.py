...
def scale_images():
    # find image path
    for file in os.listdir('MasteryManager/images'):
        # select images to scale
        if file.endswith('settings_icon.png'):
            # open image
            image = Image.open(f'MasteryManager/images/{file}')
            # resize image (values in pixels)
            image = image.resize((25, 25))
            # save image
            image.save(f'MasteryManager/images/{file}')


def assemble_image(championId:int, masteryLevel:int):
    for file in os.listdir('MasteryManager/temp/champions'):
        ...
        # open champion image
        champ_img = Image.open(f'MasteryManager/temp/champions/{file}')
        new_img = Image.new('RGBA', champ_img.size, (0, 0, 0, 0))
        
        # open mastery level image
        lvl_img = Image.open(f'MasteryManager/images/mastery_{masteryLevel}.png')
